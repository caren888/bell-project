...
int main_bck(int sys_argc, char ** sys_argv)
{
	int result;
...
int play_file(char fname)
{
	...
	//debug1("Going to play %s", strcmp(fname, "-") ? fname : "standard input");
	printf("===================Going to play %s\n", strcmp(fname, "-") ? fname : "standard input");
...
/* MinGW may have pthread installed, we prefer win32API */
	...
	{		
		...
		while (1) {
			...
			memset(playFileName, 0, sizeof(playFileName));
			get_fileName(playFileName);
			if (strlen(playFileName)) {
				fname = playFileName;
				clear_fileName();
				break;
			} else {
...
/* Rethink that SIGINT logic... */
		...
		if (intflag)
		{
			if(!skip_or_die(&start_time)) continue; //break;------
