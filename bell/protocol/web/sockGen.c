...
int socketGetInput( int sid, char* buf, int toRead, int* errCode )
{
    ...
    *errCode = 0;
    ...
    {
//		DEBUG_TEST("---main---%s,%d,%s---", __FILE__, __LINE__, __func__);		
        return 0;
    }
...
void socketProcess( int sid )
{
     ...
     * 	Process each socket
     */
    DEBUG_TEST( "---main---%s,%d,%s---socketProcess", __FILE__, __LINE__, __func__ );
    ...
    for ( ; sid < socketMax; sid++ )
    {
        ...
        if ( socketReady( sid ) )
        {
            DEBUG_TEST( "---main---%s,%d,%s---socket do event", __FILE__, __LINE__, __func__ );
            socketDoEvent( sp );
            DEBUG_TEST( "---main---%s,%d,%s---socket do event end", __FILE__, __LINE__, __func__ );
        }
    ...
    DEBUG_TEST( "---main---%s,%d,%s---socketProcess end", __FILE__, __LINE__, __func__ );
}
