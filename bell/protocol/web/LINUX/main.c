...
static int			port = 81;						/* Server port */
...
static int initWebs()
{
    ...
    websUrlHandlerDefine( T( "/goform" ), NULL, 0, websFormHandler, 0 );//这里定义form方式调用时的文件位置
    ...
    websFormDefine( T( "formTest" ), formTest );//这里定义form调用方式接口函数，注册form过程
    /*
