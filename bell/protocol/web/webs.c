...
{
    webs_t	wp;
    DEBUG_TEST( "---main---%s,%d,%s---websSocketEvent", __FILE__, __LINE__, __func__ );
    wp = ( webs_t ) iwp;
    a_assert( wp );
    ...
    {
        DEBUG_TEST( "---main---%s,%d,%s---websReadEvent", __FILE__, __LINE__, __func__ );
        websReadEvent( wp );
        DEBUG_TEST( "---main---%s,%d,%s---websReadEvent1", __FILE__, __LINE__, __func__ );
    }

    if ( mask & SOCKET_WRITABLE )
    {
        DEBUG_TEST( "---main---%s,%d,%s---websWriteEvent", __FILE__, __LINE__, __func__ );
        if ( websValid( wp ) && wp->writeSocket )
        {
            ( *wp->writeSocket )( wp );
        }
        DEBUG_TEST( "---main---%s,%d,%s---websWriteEvent1", __FILE__, __LINE__, __func__ );
    }
}
...
//        DEBUG_TEST("done2\n");
         ...
         */
//        DEBUG_TEST( "---main---%s,%d,%s---wp-state %d", __FILE__, __LINE__, __func__ , wp->state );
...
//                DEBUG_TEST("websParseFirst1\n");
                wp->state = WEBS_HEADER;
//                DEBUG_TEST("websParseFirst2\n");
                break;
            ...
            case WEBS_POST_CLEN:
                 ...
                 */
//                DEBUG_TEST("WEBS_POST_CLEN\n");
#ifndef __NO_CGI_BIN
                if ( wp->flags & WEBS_CGI_REQUEST )
                {
                    if ( fd == -1 )
                    {
//                        DEBUG_TEST("---main---%s,%d,%s---zqh0\n", __FILE__, __LINE__, __func__);
...
//                    DEBUG_TEST("zqh1\n");
					lenForTest += nbytes;
//					printf("write len: nbytes = %d, sumLen = %d\n", nbytes, lenForTest);
                    gwrite( fd, text, nbytes ); //gwrite( fd, text, gstrlen( text ) );
                    /*
...
#else
//        printf("websGetInput1\n");
        nbytes = socketRead( wp->sid, buf, len );
//        printf("websGetInput2\n");
#endif

        if ( nbytes < 0 )  						/* Error */
        {
//            DEBUG_TEST("websGetInput3\n");
            websDone( wp, 0 );
//            DEBUG_TEST("websGetInput4\n");
            return -1;
        }
...
#else
//        DEBUG_TEST("websGetInput7\n");
        nbytes = socketGets( wp->sid, &text );
//        DEBUG_TEST("websGetInput8\n");
#endif
...
#else
//            DEBUG_TEST("websGetInput9\n");
            eof = socketEof( wp->sid );
//            DEBUG_TEST("websGetInputa\n");
#endif
            ...
            if ( eof )
            {
                ...
                {
//                    DEBUG_TEST( "websGetInputc\n" );
                    websUrlHandlerRequest( wp );
//                    DEBUG_TEST( "websGetInputd\n" );
                }
...
//                    DEBUG_TEST( "websGetInputb\n" );
            ...
            {
//                DEBUG_TEST("websGetInput10\n");
                websParseRequest( wp );
                websUrlHandlerRequest( wp );
//                DEBUG_TEST("websGetInput11\n");
            }
        ...
        else if ( nbytes == 0 )
        {
            if ( wp->state == WEBS_HEADER )
            {
                 ...
                 */
//                DEBUG_TEST("websGetInput12\n");
                websUrlHandlerRequest( wp );
//                DEBUG_TEST("websGetInput13\n");
            }
...
{
	DEBUG_TEST("ssssssssssssssssssssssssssssssssssssssssss\n");
	DEBUG_TEST("ssssssssssssssssssssssssssssssssssssssssss\n");
	DEBUG_TEST("ssssssssssssssssssssssssssssssssssssssssss\n");
    char_t*		date;
    a_assert( websValid( wp ) );
...
int websWrite( webs_t wp, char_t* fmt, ... )
{
    ...
    {
		DEBUG_TEST("webs: websWrite lost data, buffer overflow\n");
