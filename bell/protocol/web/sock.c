...
int	socketRead( int sid, char* buf, int bufsize )
{
    ...
    {
//		DEBUG_TEST("---main---%s,%d,%s--- read EOF", __FILE__, __LINE__, __func__);
        return 0;
    }
...
//			DEBUG_TEST("---main---%s,%d,%s---room = %d", __FILE__, __LINE__, __func__, room);
            ...
            if ( len < 0 )
            {
                if ( errCode == EWOULDBLOCK )
                {
                    ...
                    if ( bytesRead >= 0 )
                    {
//						DEBUG_TEST("---main---%s,%d,%s--- bytesRead = %d", __FILE__, __LINE__, __func__, bytesRead);
                        return bytesRead;
                    }
                }
