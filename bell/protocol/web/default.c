                        ...
                        char_t* url, char_t* path, char_t* query )
{
    ...
    int		iRet;
    unsigned short	cgicmd = 0;
    char*		pfind = NULL;
    a_assert( websValid( wp ) );
    a_assert( url && *url );
    a_assert( path );
    a_assert( query );
    DEBUG_TEST( "---main---%s,%d,%s---cgi url:%s， pth:%s\n", __FILE__, __LINE__, __func__, url, path);
#if 0	    
	pfind = strstr( url, ".cgi" );	
    if ( pfind )
    {
        wp->cgiflag = 0x01;
        cgicmd = CgiCommand( url );
        wp->cgicmd = cgicmd;
        wp->index  = 0;
    }
	...
	DEBUG_TEST( "---main---%s,%d,%s---lpath:%s\n", __FILE__, __LINE__, __func__, lpath);
     ...
     */
    iRet = websPageOpen( wp, lpath, path, SOCKET_RDONLY | SOCKET_BINARY, 0666 );
    DEBUG_TEST( "---main---%s,%d,%s---websPageOpen=%d", __FILE__, __LINE__, __func__, iRet );
    ...
    else
    {
        ...
        bytes = sbuf.size;
		printf("date = %s, bytes = %d\n", date, bytes);
    }

	
	bytes = 0;	//added by lzg just for return success
    if ( bytes )
    {
    ...
    {
		DEBUG_TEST("---main---%s,%d,%s---All done if the browser did a HEAD request", __FILE__, __LINE__, __func__);	  
        websDone( wp, 200 );
        return 1;
    }
    ...
    if ( flags & WEBS_ASP )
    {
		DEBUG_TEST("---main---%s,%d,%s---Evaluate ASP requests", __FILE__, __LINE__, __func__);	  
     ...
     */
	DEBUG_TEST("---main---%s,%d,%s---websSetRequestSocketHandler", __FILE__, __LINE__, __func__);     
    websSetRequestSocketHandler( wp, SOCKET_WRITABLE, websDefaultWriteEvent );
#endif
    return 1;
}
    ...
    if ( !( flags & WEBS_ASP ) )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    index = wp->index;
                    len = CgiProcess( wp->cgicmd, wp->url, buf, index );

                    DEBUG_TEST("---main---%s,%d,%s---read len %d index %d\n", __FILE__ ,__LINE__, __func__, len,index);
//                    DEBUG_INFO("---main---%s,%d,%s---buf = %s", __FILE__ ,__LINE__, __func__, buf);
                    if ( len == 0x00 )
                    {
                        break;
                    }
