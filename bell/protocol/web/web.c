...
{
/* BEGIN: GOGOLINK, IPN BUG: kill encoder when IPN UnPublish ERROR */
/*        Deleted by wupm, 2015/3/18 */
#if 0
    FILE* fp = NULL;
    ...
    if ( cmd == 0x00 )
    {
        ...
        if ( fp == NULL )
        {
            printf( "web.c,104,web dbg file open failed\n" );
            return;
        }
    }
    ...
    else
    {
        ...
        if ( fp == NULL )
        {
            printf( "web.c,115,web dbg file open failed\n" );
            return;
        }
    }
...
int web_main( void )
{
    ...
    while ( !finished )
    {
        ...
        webdebug( temp, 1 );
		//DEBUG_TEST("---main---%s,%d,%s---web0\n", __FILE__, __LINE__, __func__);
        webdebug( "web0\n", 0 );

        if ( socketReady( -1 ) || socketSelect( -1, 1000 ) )
        {
            DEBUG_TEST("---main---%s,%d,%s---web1\n", __FILE__, __LINE__, __func__);
            webdebug( "web1\n", 0 );
            socketProcess( -1 );
            DEBUG_TEST("---main---%s,%d,%s---web2\n", __FILE__, __LINE__, __func__);
            //printf("web2\n");
        }

        //printf("web3\n");
		//DEBUG_TEST("---main---%s,%d,%s---web3\n", __FILE__, __LINE__, __func__);
        websCgiCleanup();
		//DEBUG_TEST("---main---%s,%d,%s---web4\n", __FILE__, __LINE__, __func__);
        //printf("web4\n");
		//DEBUG_TEST("---main---%s,%d,%s---web5\n", __FILE__, __LINE__, __func__);
        emfSchedProcess();
		//DEBUG_TEST("---main---%s,%d,%s---web6\n", __FILE__, __LINE__, __func__);
        //printf("web5\n");
    }
...
//	get_net_para();
	...
	DEBUG_ERR("webdir=%s  dir=%s  rootweb=%s", webdir, dir, rootWeb);
    /*
...
//    pname2 = bstrdup( B_L, pname1 );
	pname2 = bstrdup( B_L, "/tmp/post.bin" );
    free( pname1 );
    return pname2;
}
